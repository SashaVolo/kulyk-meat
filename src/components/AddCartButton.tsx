...
interface Props {
  large?: boolean;
  product?: any; // Приймаємо будь-який формат товару
}
...
export default function AddToCartButton({ large = false, product }: Props) {
    ...
    if (product) {
      ...
      if (product.image && typeof product.image === "object") {
        try {
           ...
           console.error("Помилка генерації URL картинки:", error);
           imageUrl = ""; 
        }
      }
      ...
      const itemToCart = {
        _id: product._id,
        uniqueId: product.uniqueId || product._id, // Унікальний ID
        title: product.title,
        price: product.price,
        image: imageUrl, 
        quantity: 1
  ...
  return (
    ...
    >
      ...
      {large && <span>{isAdded ? "Додано!" : "Замовити"}</span>}
    </button>
  );
}